...
for steps = 2:N
    ...
    Combinations = nchoosek(U',steps);%生成U中地点能组成的所有组合
    TOld = T;
    T = [];
    ...
    for j = 1:size(Combinations,1)
        if ct > nRules
            ...
            if sum(ismember(nchoosek(Combinations(j,:),steps-1),TOld,'rows')) - steps+1>0
                ...
                if S >= minSup
                    T = [T; Combinations(j,:)];%生成新的pattern
...
FreqItemsets(steps-1:end) = [];
RuleConf = RuleConf(1:ct-1);%删除多余的0项
...
switch sortFlag
    case 1 % Sort by Support level
        [V ind] = sort(RuleSup,'descend');%ind每个点的位置
...
disp(['关联规则算法完成,规则数为：' num2str(size(RuleSup,1))]);%把数值转换成字符串

% Save the rule in a text file and print them on display
fid = fopen(rulesfile, 'w');
fprintf(fid, '%s   (%s, %s, %s, %s) \r\n', 'Rule', 'Support', 'F', 'Confidence', 'F');

for i = 1:size(Rules{1},1)
    s1 = '';
    s2 = '';
    for j = 1:size(Rules{1}{i},2)  %生成前项
    ...
    Fsup=num2str((RuleSup(i)*M));
    Fconf=num2str(RuleSup(i)*M*RuleConf(i));
    fprintf(fid, '%s %s %s%% %s %s%% %s \r\n', s1, s2, s3, Fsup, s4, Fconf);%写入文件
end
fclose(fid);
